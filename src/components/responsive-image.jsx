...
const ResponsiveImage = ({ src, alt, children }) => {
	...
	return (
		<div
			className={`relative ${
				isSmallScreen ? 'w-full h-screen' : 'w-full xs:w-10 sm:w-full md:w-full lg:w-full md:h-auto'
			} flex items-center justify-center overflow-hidden`}>
			<Image
				src={src}
				alt={alt}
				layout={isSmallScreen ? 'fill' : 'intrinsic'}
				objectFit={isSmallScreen ? 'cover' : 'contain'}
				className={`${isSmallScreen ? 'scale-100' : 'scale-100'}`}
			/>
			{isSmallScreen && (
